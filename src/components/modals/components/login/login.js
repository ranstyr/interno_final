export class LoginController {
  ...
  /* @ngInject */
  constructor($modalInstance, Auth, PATTERNS) {
  ...
  login() {
    ...
    this.Auth.login(this.credentials)
      .then(userData => this.closeModal(userData))
      .catch((error) => {
        if (error.code === 1009) { // User requires confirmation
          this.dismissModal('confirmCode');
        }
